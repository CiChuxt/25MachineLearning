...
def save_embedding(y, regions):
    ...
    ax.scatter(y[:, 0], y[:, 1], s=42, color="#ef5675")
    ...
    ax.set_title(f"ISOMAP二维嵌入图(k={N_NEIGHBORS})")
    fig.tight_layout()
    fig.savefig(FIG_DIR / "isomap_embedding.png", bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    pd.DataFrame({"地区": regions, "ISOMAP1": y[:, 0], "ISOMAP2": y[:, 1]}).to_csv(
        "isomap_embedding.csv", index=False, encoding="utf-8-sig"
    )
