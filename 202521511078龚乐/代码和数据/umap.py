...
def save_embedding(y, regions):
    ...
    ax.scatter(y[:, 0], y[:, 1], s=42, color="#00876c")
    for x0, y0, name in zip(y[:, 0], y[:, 1], regions):
        ax.annotate(name, (x0, y0), xytext=(4, 3), textcoords="offset points", fontsize=8)
    ax.set_xlabel("UMAP1")
    ax.set_ylabel("UMAP2")
    ax.set_title(f"UMAP二维嵌入图(n_neighbors={N_NEIGHBORS}, min_dist={MIN_DIST})")
    fig.tight_layout()
    fig.savefig(FIG_DIR / "umap_embedding.png", bbox_inches="tight")
    plt.close(fig)
...
def main():
    ...
    pd.DataFrame({"地区": regions, "UMAP1": y[:, 0], "UMAP2": y[:, 1]}).to_csv(
        "umap_embedding.csv", index=False, encoding="utf-8-sig"
    )
    save_embedding(y, regions)
    print("UMAP completed.")
    print(f"n_neighbors={N_NEIGHBORS}, min_dist={MIN_DIST}, random_state={RANDOM_STATE}")
