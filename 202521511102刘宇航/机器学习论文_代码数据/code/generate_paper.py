...
def set_font(run, font_name='宋体', size=10.5, bold=False, english_font='Times New Roman'):
...
def add_heading(doc, text, level=1):
    ...
    if level == 1:
        set_font(run, font_name='黑体', size=16, bold=True)
    elif level == 2:
        set_font(run, font_name='黑体', size=14, bold=True)
    elif level == 3:
        set_font(run, font_name='黑体', size=12, bold=True)
    return heading
...
def add_paragraph(doc, text, indent=True):
    ...
    run = p.add_run(text)
    set_font(run, font_name='宋体', size=10.5)
    return p
...
def add_figure(doc, img_path, caption):
    ...
    set_font(run, font_name='宋体', size=10.5, bold=False)
...
def add_table(doc, data, headers, caption):
    ...
    for i, header in enumerate(headers):
        ...
        set_font(run, font_name='宋体', size=10.5, bold=True)
...
def generate_paper():
    ...
    set_font(run, font_name='黑体', size=22, bold=True)
    ...
    for label, value in info_lines:
        ...
        set_font(run1, font_name='宋体', size=14)
        run2 = p.add_run(value)
        set_font(run2, font_name='宋体', size=14)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('max_w  w^T Σw,  s.t.  w^T w = 1')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, '其中 Σ 为数据的协方差矩阵，w 为投影方向。使用拉格朗日乘数法求解：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('L(w, λ) = w^T Σw - λ(w^T w - 1)')
    set_font(run, font_name='Times New Roman', size=10.5)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('Cumulative Variance Ratio = (Σᵢ₌₁ᵏ λᵢ) / (Σᵢ₌₁ᵈ λᵢ)')
    set_font(run, font_name='Times New Roman', size=10.5)
    ...
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('p_{j|i} = exp(-||x_i - x_j||² / 2σ_i²) / Σ_{k≠i} exp(-||x_i - x_k||² / 2σ_i²)')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, '其中σ_i是以x_i为中心的高斯分布的方差，通过困惑度（Perplexity）参数来调节。困惑度定义为：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('Perp(P_i) = 2^{H(P_i)},  H(P_i) = -Σ_j p_{j|i} log₂ p_{j|i}')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, '为了对称化处理，定义联合概率分布：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('p_{ij} = (p_{j|i} + p_{i|j}) / (2n)')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, '在低维空间中，对应的点为y_i和y_j，使用自由度为1的t分布计算相似度q_{ij}：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('q_{ij} = (1 + ||y_i - y_j||²)^{-1} / Σ_{k≠l} (1 + ||y_k - y_l||²)^{-1}')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, 't-SNE的目标是最小化高维分布P和低维分布Q之间的KL散度：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('C = KL(P||Q) = Σ_i Σ_j p_{ij} log(p_{ij} / q_{ij})')
    set_font(run, font_name='Times New Roman', size=10.5)
    
    add_paragraph(doc, '使用梯度下降法优化该目标函数。对y_i求梯度可得：')
    p = doc.add_paragraph()
    p.alignment = WD_ALIGN_PARAGRAPH.CENTER
    run = p.add_run('∂C/∂y_i = 4 Σ_j (p_{ij} - q_{ij}) (y_i - y_j) (1 + ||y_i - y_j||²)^{-1}')
    set_font(run, font_name='Times New Roman', size=10.5)
